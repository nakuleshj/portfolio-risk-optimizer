...
tickers = st.multiselect("Select Tickers", all_tickers, default=['AAPL','XOM', 'JPM', 'GLD'], max_selections=5)
...
initial_capital = st.sidebar.number_input("Enter Value of Starting Capital:",min_value=1000, max_value=100000, value=10000)
rf = st.sidebar.slider("Risk-free Rate", min_value=0.00, max_value=5.00, step=0.01, format="%.2f", value=rf*100)/100
n_rand_pf = st.sidebar.slider('No. of Random Portfolios', min_value=500, max_value=10000, step=500, value=10000)
...
with tabs[1]:
      ...
      mpt_fig = px.scatter(random_pfs, 
                           x="Volatility", 
                           y="Return", 
                           color="Sharpe",
                           color_continuous_scale="purples")
      mpt_fig.add_trace(
            go.Scattergl(x=[msr_pf["Volatility"]], y=[msr_pf["Return"]],mode='markers', 
                    marker=dict(color='red', symbol='diamond', size = 10, line=dict(width=2,
                                        color='black')), 
                    name='MSR Portfolio'))
      mpt_fig.add_trace(
            go.Scattergl(x=[gmv_pf["Volatility"]], y=[gmv_pf["Return"]],mode='markers', 
                    marker=dict(color='green', symbol='square', size = 10, line=dict(width=2,
                                        color='black')), 
                    name='GMV Portfolio'))
      mpt_fig.update_layout(coloraxis_colorbar=dict(yanchor="top", y=.8, x=1.02))
      st.plotly_chart(mpt_fig)    

      st.subheader("Portfolio Analysis")

      pf_selection = st.selectbox("Pick a portfolio",['Equally Weighted','Max Sharpe Ratio (MSR)', 'Global Minimum Volatility (GMV)'], index=0)
      ...
      for i, k in enumerate(pf_map.keys()):
            ...
            alloc_fig = px.pie(selected_pf, names=selected_pf.index, values=selected_pf.values, title=k)
            st.plotly_chart(alloc_fig)

      st.subheader(f"Growth of Hypothetical ${initial_capital:,}")
      pf_returns = pf.calc_pf_returns(asset_returns, pf_map.get(pf_selection)[tickers])
      pf_returns.name = pf_selection
      equity_curve_pf = assets.equity_curve(pf_returns, initial_capital)

      drawdowns_pf = assets.drawdowns(pf_returns)

      ec_fig = px.line(equity_curve_pf)
      ec_fig.update_traces(line_color="#18A838")
      ec_fig.update_layout(xaxis_title="Date", 
                    yaxis_title="Value of Investment (USD)",
                    showlegend=False)
      st.plotly_chart(ec_fig)
      ...
      dd_fig = px.area(drawdowns_pf)
      dd_fig.update_traces(stackgroup = None, fill = 'tozeroy', line_color='red')
      dd_fig.update_layout(xaxis_title="Date", yaxis_title="Drawdowns (%)", showlegend=False)
      st.plotly_chart(dd_fig)
